...
def get_image_paths(root_dir):
    ...
    for first_level in sorted(os.listdir(root_dir), key=lambda x: int(x) if x.isdigit() else float('inf')):
        ...
        labeled_path = os.path.join(first_level_path, "legend")
        ...
        if image_files:
            grouped_paths[first_level_path].extend(image_files)
    ...
    for img_dir, img_paths in tqdm(image_dict.items()):
        for k, image_path in enumerate(img_paths):
            ...
            if not args.only_pred_file:
                ...
                #safe_imwrite(mask_and_box_image_file, mask_and_box_image)
